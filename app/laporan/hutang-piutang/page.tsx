...
import { FileSpreadsheet, Printer, Search, ExternalLink, UserCheck, Landmark, HardHat, Receipt } from 'lucide-react';
...
export default function LaporanHutangPiutangPage() {
  ...
  const saleCalculations = useMemo(() => {
    const map = new Map<string, { totalTagihan: number; totalBayar: number; sisa: number; terakhirBayar: string }>();
    ...
    sales.forEach((s) => {
      ...
      const totalAddCosts = addCosts.reduce((sum, c) => sum + (c.nominal || 0), 0);
      ...
      if (payments.length > 0) {
        const sortedDates = [...payments].sort((a, b) => new Date(b.tanggal).getTime() - new Date(a.tanggal).getTime());
  ...
  const kasbonList = useMemo(() => {
    const grouped: Record<string, { nama: string; no_hp: string; instansi: string; merekaHutang: number; perusahaanHutang: number }> = {};
  ...
  const totalKasbonMereka = useMemo(() => filteredKasbon.reduce((acc, curr) => acc + curr.merekaHutang, 0), [filteredKasbon]);
  const totalKasbonPerusahaan = useMemo(() => filteredKasbon.reduce((acc, curr) => acc + curr.perusahaanHutang, 0), [filteredKasbon]);

  // --- 2. Penjualan Unit Cash ---
  const cashSalesList = useMemo(() => {
    const activeCashSales = sales.filter(
      (s) => s.status !== 'Batal' && s.metode_bayar && s.metode_bayar.toLowerCase().includes('cash')
    );

    return activeCashSales.map((s) => {
      const cust = customers.find((c) => c.id === s.customer_id);
      const calc = saleCalculations.get(s.id) || { totalTagihan: 0, totalBayar: 0, sisa: 0, terakhirBayar: '-' };
  ...
  const totalCashMereka = useMemo(() => filteredCashSales.reduce((acc, curr) => acc + curr.merekaHutang, 0), [filteredCashSales]);
  const totalCashPerusahaan = useMemo(() => filteredCashSales.reduce((acc, curr) => acc + curr.perusahaanHutang, 0), [filteredCashSales]);

  // --- 3. Penjualan Unit KPR (Cust) ---
  const kprCustSalesList = useMemo(() => {
    const activeKprSales = sales.filter(
      (s) => s.status !== 'Batal' && s.metode_bayar && s.metode_bayar.toUpperCase() === 'KPR'
    );
  ...
  const totalKprCustMereka = useMemo(() => filteredKprCust.reduce((acc, curr) => acc + curr.merekaHutang, 0), [filteredKprCust]);
  const totalKprCustPerusahaan = useMemo(() => filteredKprCust.reduce((acc, curr) => acc + curr.perusahaanHutang, 0), [filteredKprCust]);

  // --- 4. Penjualan Unit KPR (Bank) ---
  const kprBankList = useMemo(() => {
    const bankGroup: Record<string, { bankId: string; user: string; noTelp: string; instansi: string; alamat: string; merekaHutang: number; perusahaanHutang: number }> = {};
    ...
    sales.forEach((s) => {
      if (s.status !== 'Batal' && s.metode_bayar === 'KPR' && s.bank_id && bankGroup[s.bank_id]) {
        const nominalKpr = s.kredit_pengajuan || 0;
        if (s.kpr_status === 'Akad' || s.kpr_status === 'ACCEPTED' || s.kpr_status === 'SP3K') {
    ...
    return Object.values(bankGroup).filter((b) => b.merekaHutang > 0 || b.perusahaanHutang > 0);
  ...
  const totalKprBankMereka = useMemo(() => filteredKprBank.reduce((acc, curr) => acc + curr.merekaHutang, 0), [filteredKprBank]);
  const totalKprBankPerusahaan = useMemo(() => filteredKprBank.reduce((acc, curr) => acc + curr.perusahaanHutang, 0), [filteredKprBank]);

  // --- 5. Operasional ---
  const operasionalList = useMemo(() => {
    const grouped: Record<string, { user: string; noTelp: string; instansi: string; alamat: string; merekaHutang: number; perusahaanHutang: number }> = {};
  ...
  const totalOperasionalMereka = useMemo(() => filteredOperasional.reduce((acc, curr) => acc + curr.merekaHutang, 0), [filteredOperasional]);
  const totalOperasionalPerusahaan = useMemo(() => filteredOperasional.reduce((acc, curr) => acc + curr.perusahaanHutang, 0), [filteredOperasional]);

  // --- Summary Top Cards ---
  const grandTotalMerekaHutang = useMemo(() => {
    return totalKasbonMereka + totalCashMereka + totalKprCustMereka + totalKprBankMereka + totalOperasionalMereka;
  }, [totalKasbonMereka, totalCashMereka, totalKprCustMereka, totalKprBankMereka, totalOperasionalMereka]);

  const grandTotalPerusahaanHutang = useMemo(() => {
    return totalKasbonPerusahaan + totalCashPerusahaan + totalKprCustPerusahaan + totalKprBankPerusahaan + totalOperasionalPerusahaan;
  }, [totalKasbonPerusahaan, totalCashPerusahaan, totalKprCustPerusahaan, totalKprBankPerusahaan, totalOperasionalPerusahaan]);

  // --- Drill Down Click Handler ---
  const handleOpenDetail = (name: string, type: 'customer' | 'bank' | 'mandor' | 'operasional', specificCustId?: string, specificBankId?: string) => {
    let instansi = '-';
    let no_hp = '-';
    let alamat = '-';
    let diaHutang = 0;
    ...
    let custObj = customers.find((c) => c.nama.toLowerCase() === name.toLowerCase() || c.id === specificCustId);
    let bankObj = banks.find((b) => (b.nama_bank + (b.cabang ? ` ${b.cabang}` : '')).toLowerCase() === name.toLowerCase() || b.id === specificBankId);
    ...
    const userOperasional = operationalExpenses
      .filter((oe) => (oe.keterangan && oe.keterangan.toLowerCase().includes(name.toLowerCase())) || (oe.kategori && oe.kategori.toLowerCase().includes(name.toLowerCase())))
      .map((oe) => ({ tanggal: oe.tanggal, keterangan: oe.keterangan || oe.kategori, sisa: oe.nominal }));

    const userSales = sales
      .filter((s) => {
        if (custObj && s.customer_id === custObj.id) return true;
        if (bankObj && s.bank_id === bankObj.id) return true;
        if (s.customer_nama && s.customer_nama.toLowerCase() === name.toLowerCase()) return true;
        return false;
      })
      .map((s) => {
  ...
  const handleExportExcelAll = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filteredKasbon), 'Kasbon_Mandor');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filteredCashSales), 'Penjualan_Cash');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filteredKprCust), 'KPR_Konsumen');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filteredKprBank), 'KPR_Bank');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filteredOperasional), 'Operasional');
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Rekapitulasi piutang konsumen, kasbon mandor, hutang bank &amp; operasional perusahaan
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-6 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl text-white shadow-lg relative overflow-hidden">
            ...
            <p className="text-sm font-semibold uppercase tracking-wider opacity-90">Mereka Hutang (Piutang)</p>
          ...
          <div className="p-6 bg-gradient-to-br from-rose-500 to-red-600 rounded-xl text-white shadow-lg relative overflow-hidden">
            ...
            <p className="text-sm font-semibold uppercase tracking-wider opacity-90">Perusahaan Hutang (Kewajiban)</p>
            <p className="text-3xl sm:text-4xl font-extrabold mt-2 tracking-tight">
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full p-6 space-y-6 animate-in fade-in zoom-in duration-200">
              <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-800">List Hutang Piutang</h2>
                  <p className="text-xs text-slate-400">Detail rincian tagihan &amp; kewajiban per pengguna / bank</p>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg border border-slate-200 text-xs text-slate-700">
                <div className="space-y-1.5">
                  <p><span className="font-semibold text-slate-500 inline-block w-28">Nama:</span> <span className="font-bold text-slate-800 text-sm">{selectedEntity.nama}</span></p>
                  <p><span className="font-semibold text-slate-500 inline-block w-28">Instansi:</span> <span>{selectedEntity.instansi}</span></p>
                  <p><span className="font-semibold text-slate-500 inline-block w-28">No Handphone:</span> <span className="font-mono">{selectedEntity.no_hp}</span></p>
                  <p><span className="font-semibold text-slate-500 inline-block w-28">Alamat:</span> <span>{selectedEntity.alamat}</span></p>
                </div>
                <div className="flex flex-col justify-center gap-2 border-t md:border-t-0 md:border-l border-slate-200 pt-3 md:pt-0 md:pl-4">
                  <div className="bg-red-50 border border-red-200 p-2.5 rounded-md">
                    <p className="text-slate-500 font-semibold text-[11px]">Perusahaan Hutang</p>
                    <p className="text-lg font-bold text-red-600">{formatRupiah(selectedEntity.perusahaanHutang)}</p>
                  </div>
                  <div className="bg-emerald-50 border border-emerald-200 p-2.5 rounded-md">
                    <p className="text-slate-500 font-semibold text-[11px]">Dia Hutang (Mereka Hutang)</p>
                    <p className="text-lg font-bold text-emerald-600">{formatRupiah(selectedEntity.diaHutang)}</p>
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full text-xs text-left text-slate-600">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {selectedEntity.kasbonMandor.length === 0 ? (
                        <tr><td colSpan={2} className="p-3 text-center text-slate-400">Tidak ada data kasbon mandor.</td></tr>
                      ) : (
                        selectedEntity.kasbonMandor.map((km, idx) => (
                          <tr key={idx} className="hover:bg-slate-50">
                            <td className="p-2.5">{km.tanggal}</td>
                            <td className="p-2.5 text-right font-bold text-slate-800">{formatRupiah(km.sisa)}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
                  ...
                  <table className="w-full text-xs text-left text-slate-600">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {selectedEntity.operasional.length === 0 ? (
                        <tr><td colSpan={3} className="p-3 text-center text-slate-400">Tidak ada data operasional.</td></tr>
                      ) : (
                        selectedEntity.operasional.map((op, idx) => (
                          <tr key={idx} className="hover:bg-slate-50">
                            <td className="p-2.5">{op.tanggal}</td>
                            <td className="p-2.5">{op.keterangan}</td>
                            <td className="p-2.5 text-right font-bold text-slate-800">{formatRupiah(op.sisa)}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
                  ...
                  <table className="w-full text-xs text-left text-slate-600">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {selectedEntity.penjualanUnit.length === 0 ? (
                        <tr><td colSpan={5} className="p-3 text-center text-slate-400">Tidak ada transaksi penjualan unit.</td></tr>
                      ) : (
                        selectedEntity.penjualanUnit.map((pu) => (
                          <tr key={pu.id} className="hover:bg-slate-50">
                            <td className="p-2.5 font-medium">{pu.tanggal}</td>
                            <td className="p-2.5 font-bold text-slate-800">{pu.unit}</td>
                            <td className="p-2.5 text-slate-500">{pu.komitmen}</td>
                            <td className="p-2.5 text-right font-bold text-emerald-600">{formatRupiah(pu.sisa)}</td>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-800 text-base">Kasbon Mandor</h3>
              <p className="text-xs text-slate-400">Rekapitulasi sisa kasbon mandor proyek</p>
            </div>
            ...
            <table className="w-full text-left text-xs text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredKasbon.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-6 text-center text-slate-400">Belum ada data kasbon mandor.</td>
                  </tr>
                ) : (
                  filteredKasbon.map((item, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                      ...
                      <td className="py-2.5 px-4 text-right font-bold text-emerald-600">{formatRupiah(item.merekaHutang)}</td>
                      <td className="py-2.5 px-4 text-right font-bold text-rose-600">{formatRupiah(item.perusahaanHutang)}</td>
                    </tr>
                  ))
                )}
              </tbody>
              <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-slate-800">
                <tr>
                  <td colSpan={3} className="py-3 px-4 uppercase text-xs">Total Kasbon Mandor</td>
                  <td className="py-3 px-4 text-right text-emerald-600">{formatRupiah(totalKasbonMereka)}</td>
                  <td className="py-3 px-4 text-right text-rose-600">{formatRupiah(totalKasbonPerusahaan)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-800 text-base">Penjualan Unit Cash</h3>
              <p className="text-xs text-slate-400">Daftar sisa tagihan penjualan unit metode Cash / Cash Bertahap</p>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
              <input
            ...
            <table className="w-full text-left text-xs text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredCashSales.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-6 text-center text-slate-400">Belum ada data penjualan unit cash.</td>
                  </tr>
                ) : (
                  filteredCashSales.map((item) => (
                    <tr key={item.saleId} className="hover:bg-blue-50/50 transition-colors">
                      <td className="py-2.5 px-4">
                        <button
                          onClick={() => handleOpenDetail(item.user, 'customer', item.customerId)}
                          className="font-bold text-blue-600 hover:underline text-left"
                        >
                          {item.user}
                        </button>
                      ...
                      <td className="py-2.5 px-4">{item.instansi}</td>
                      <td className="py-2.5 px-4 text-center font-mono">{item.terakhirBayar}</td>
                      <td className="py-2.5 px-4 text-right font-bold text-emerald-600">{formatRupiah(item.merekaHutang)}</td>
                      <td className="py-2.5 px-4 text-right font-bold text-rose-600">{formatRupiah(item.perusahaanHutang)}</td>
                    </tr>
                  ))
              ...
              <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-slate-800">
                <tr>
                  <td colSpan={4} className="py-3 px-4 uppercase text-xs">Total Penjualan Unit Cash</td>
                  <td className="py-3 px-4 text-right text-emerald-600">{formatRupiah(totalCashMereka)}</td>
                  <td className="py-3 px-4 text-right text-rose-600">{formatRupiah(totalCashPerusahaan)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-800 text-base">Penjualan Unit KPR (Cust)</h3>
              <p className="text-xs text-slate-400">Tagihan porsi konsumen (DP / sisa non-KPR) untuk transaksi KPR</p>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
              <input
            ...
            <table className="w-full text-left text-xs text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredKprCust.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-6 text-center text-slate-400">Belum ada data piutang KPR konsumen.</td>
                  </tr>
                ) : (
              ...
              <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-slate-800">
                <tr>
                  <td colSpan={4} className="py-3 px-4 uppercase text-xs">Total Penjualan Unit KPR (Cust)</td>
                  <td className="py-3 px-4 text-right text-emerald-600">{formatRupiah(totalKprCustMereka)}</td>
                  <td className="py-3 px-4 text-right text-rose-600">{formatRupiah(totalKprCustPerusahaan)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-800 text-base">Penjualan Unit KPR (Bank)</h3>
              <p className="text-xs text-slate-400">Plafon akad KPR yang akan / sedang diproses pencairannya oleh Bank</p>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
              <input
            ...
            <table className="w-full text-left text-xs text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredKprBank.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-6 text-center text-slate-400">Belum ada data KPR Bank.</td>
                  </tr>
                ) : (
                  filteredKprBank.map((item) => (
                    <tr key={item.bankId} className="hover:bg-blue-50/50 transition-colors">
                      <td className="py-2.5 px-4">
                        <button
                          onClick={() => handleOpenDetail(item.user, 'bank', undefined, item.bankId)}
                          className="font-bold text-blue-600 hover:underline text-left"
                        >
                          {item.user}
                        </button>
              ...
              <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-slate-800">
                <tr>
                  <td colSpan={3} className="py-3 px-4 uppercase text-xs">Total Penjualan Unit KPR (Bank)</td>
                  <td className="py-3 px-4 text-right text-emerald-600">{formatRupiah(totalKprBankMereka)}</td>
                  <td className="py-3 px-4 text-right text-rose-600">{formatRupiah(totalKprBankPerusahaan)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-800 text-base">Operasional</h3>
              <p className="text-xs text-slate-400">Kewajiban &amp; piutang pengeluaran operasional proyek</p>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
              <input
            ...
            <table className="w-full text-left text-xs text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredOperasional.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-6 text-center text-slate-400">Belum ada data hutang operasional.</td>
                  </tr>
                ) : (
                  filteredOperasional.map((item, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                      <td className="py-2.5 px-4">
                        <button
                          onClick={() => handleOpenDetail(item.user, 'operasional')}
                          className="font-bold text-blue-600 hover:underline text-left"
                        >
                          {item.user}
                        </button>
              ...
              <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-slate-800">
                <tr>
                  <td colSpan={3} className="py-3 px-4 uppercase text-xs">Total Operasional</td>
                  <td className="py-3 px-4 text-right text-emerald-600">{formatRupiah(totalOperasionalMereka)}</td>
                  <td className="py-3 px-4 text-right text-rose-600">{formatRupiah(totalOperasionalPerusahaan)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
