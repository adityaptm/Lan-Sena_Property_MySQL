...
export default function PrintKomplenPage() {
  ...
  useEffect(() => {
    async function generatePdf() {
      ...
      try {
        ...
        const { SuratKomplenDocument } = await import('@/components/pdf/SuratKomplenDocument');
  ...
  return (
    <AppLayout>
      <div className="mb-4">
        <h1 className="text-xl font-bold text-slate-800">Cetak Surat Komplen Konsumen</h1>
        <p className="text-sm text-slate-500">Pratinjau Dokumen Surat Komplen</p>
      </div>
      ...
      <div className="w-full h-[800px] border border-slate-300 rounded overflow-hidden shadow-lg">
        {isLoading ? (
          ...
          <iframe src={pdfUrl} className="w-full h-full" title="Surat Komplen Document" />
        ) : (
